...
cat $neighbors | while read neighbor; do
	...
	if [ ! -z $CDS_sbjct ]; then
		...
		if [ ! -s ${process_dir}/${neighbor}_CDS_elongated.nfasta ]; then
			...
			python ANC_ORF/SCRIPTS/ORFget_v2.py -fna GENOMES/${neighbor}.fna -gff GENOMES/${neighbor}.gff -o ${process_dir}/${neighbor}_CDS -type nfasta -features_include CDS -elongate 100
		fi
...
if [ $seq_nb -gt 2 ]; then
	...
	python ANC_ORF/SCRIPTS/prune_tree.py -tree ${process_dir}/${tree} -names <(echo "$names") -out ${process_dir}/${query}.nwk
else
