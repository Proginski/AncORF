...
# - wheter of not to perform a multiframe translation (multiframe)
...
if [ ! -s ${outdir}/${name}_CDS.faa ]; then
	...
	gffread -V -g $gfasta -x ${outdir}/${name}_CDS.fna.tmp $gff
	
	#linearize the fna
	awk '/^>/ {printf("%s%s\n",(N>0?"\n":""),$0);N++;next;} {printf("%s",$0);} END {printf("\n");}' ${outdir}/${name}_CDS.fna.tmp > ${outdir}/${name}_CDS.fna
	rm ${outdir}/${name}_CDS.fna.tmp
...
if [ $multiframe == "True" ]; then
	...
	' ${pref_suf}.faa ${outdir}/${name}_CDS.faa >> ${outdir}/${name}_CDS_multielongated.faa
else
	awk '
	BEGIN{FS=OFS="\t"}
	{
	...
	' ${pref_suf}.faa ${outdir}/${name}_CDS.faa >> ${outdir}/${name}_CDS_elongated.faa
fi


# Finally, also buil an elongated CDS FASTA.
